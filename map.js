...
(function() {	
	...
	var canvas = document.getElementById('terrain2');
	...
	//screen
	var screen = new Screen(20,10, 128, 128, '2d');
			...
			for(j = 0 ; j < this.h; j++){
				for( i = 0 ; i < this.w; i++) {
					...
					switch(terrain) {
						...
						case 'forrest': r = 255; g = 255; b = 0;break;						
					}
					d[0]   = r;
					d[1]   = g;
					d[2]   = b;
	...
	function Screen(x,y, cellw, cellh, mode) {
		...
		this.moveTo = function(x,y) { console.log('X:' + x + ' Y:' + y);
			cursor.absx = x;
			cursor.absy = y;
			console.log(x + ' ' + y);
			var w05; var h05;
		...
		//draws screen;
		this.draw = function() {
			ctx.clearRect(0, 0, 3024, 1512);
			ctx2.clearRect(0, 0, 3024, 1512);
		...
		this.draw2dCell = function(x,y) {
			...
			/** if forrest or hill or mountains - draw on second layer **/
			switch(terrain) {
				case 'forrest': terrain2 = 'forrest'; terrain = 'grassland'; break;
				case 'hills': terrain2 = 'hills'; terrain = 'plains'; break;
				case 'mountains': terrain2 = 'mountains'; terrain = 'plains'; break;
			}
			console.log(terrain2);
			ctx.drawImage(resources.get(imgprefix + terrain + imgext), tx,ty);
			if(terrain2 !== '') ctx2.drawImage(resources.get(imgprefix + terrain2 + imgext), tx,ty);
		}
		...
		this.draw2dCursor = function(){
			...
			ctx2.rect(tx, ty, this.cellw, this.cellh);
			ctx2.stroke();
			//console.log(tx + ' ' + ty);
